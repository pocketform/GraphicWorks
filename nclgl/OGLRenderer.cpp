...
void OGLRenderer::UpdateShaderMatrices()	{
	if(currentShader) {
		...
		glUniformMatrix4fv(glGetUniformLocation(currentShader->GetProgram(), "shadowMatrix"),   1,false, (float*)&shadowMatrix);//add for shadow
	}
}
...
void OGLRenderer::SetShaderLight(const Light &l) {
	...
	//try to add multiple lights
	//for (int i = 0; i < 4; i++)
	//{
	//	stringstream stream;
	//	string num;

	//	string lightPos = "lightPos";
	//	string lightColour = "lightColour";
...
//	Vector4 sdomething[4];

//	"[" + tostring(i) + "]"
}

void OGLRenderer::SetShaderLight1(const Light &l)
{
	for (int i = 0; i < 4; i++)
	{
		glUniform3fv(glGetUniformLocation(currentShader->GetProgram(), "lightPos1"),    1, (float*)&l.GetPosition());
